...
## Batch 2 additions (V2) — states & flows

### New per-screen states

**Character Dossier:** living · fallen (respectful, desaturated, sheet shows 0/…) · sparse
(recently introduced — graceful partial state, "little is known yet") · narrow (sheet zone stacks
below the story-register sections).

**Role Matrix (in Settings & Wizard):** recommended-default (badged) · manually-overridden (marker
+ per-row reset) · json-mode-risk (caution line under a structured role whose model can't guarantee
structured output) · provider model-list loading · free-text model entered (Advanced badge).

**Sampler panel:** recommended-default (pre-filled, matches a preset) · custom (edited, "Custom"
marker + reset) · provider-limited (some fields disabled with a note) · collapsed summary in the
row ("Creative · temp 0.8").

**Lorebook:** library empty (invitation) · library populated · entry editor (keys, content,
enabled, always-on, priority). **Story Settings attach:** none attached · attached-enabled ·
attached-but-disabled · imported-from-card (badged, auto-attached).

**Blueprint editor:** creating-new · editing-existing (imported cards show "Imported · Card V2/V3"
provenance) · long-content (scrolling fields) · validation (missing name).

**Play message controls:** idle (controls hidden until hover/focus) · latest message (swipe + ⋯) ·
earlier message (rewind-only ⋯) · swiping/regenerating (in-voice "retelling…") · variant cycling
(counter changes, dice do not) · locked-die shown when the turn had a ruling · delete/rewind confirm
· reduced-motion.

### Updated flow — configuring models (first run & Settings)
Wizard step 3 and Settings share the RoleMatrix. Default state already shows the recommended model
+ recommended sampler profile per role (no blank state). A user can accept in one click, swap a
provider (triggers a model-list load, then a role-aware recommendation set), pick a model, or type
a model ID. Choosing a model that can't guarantee structured output on a structured role
(Classifier / Analyzer / Story AI) surfaces the json-mode-risk caution.

### Acceptance additions (Batch 2)
- [ ] RoleMatrix interactive (provider→role-aware model + free-text) and identical in Wizard & Settings.
- [ ] Global Lorebook library + Story Settings attach/detach with per-story enable.
- [ ] Blueprint editor full field set incl. advanced narration group + "rules can't be overridden" copy.
- [ ] Persona pickable at story creation and changeable in Story Settings.
- [ ] Latest message swipe + delete/rewind; earlier = rewind-only; "roll stands" copy + locked-die present.
- [ ] Character Dossier with both-register sections, observations timeline, dual-direction relationships.
- [ ] Full per-role Sampler panel with presets, pre-filled from recommended profile; per-role reset.
- [ ] Two-register discipline, tokens, and Ruling animation unchanged.


---

## V3 additions — states, routing & acceptance

### Setup Wizard states
first-launch (fresh, step 1) · validating (key spinner) · rejected (key + reason) · incomplete
(tried to advance without meeting a criterion — blocking message) · completed (both criteria met →
success) · provider-removed (previously complete, provider later gone → reconnect warning) ·
dismissed (skipped → Library banner persists).

### Library provider-configuration states
configured (normal) · unconfigured (persistent banner; New Story + play route to setup; Import &
browse stay) · returning-from-dismiss (banner still shown).

### Custom endpoint states
idle · bad-url (inline) · validating · valid (+ discovered models) · rejected (401) · network-error
· no-models (→ manual model-ID entry).

### Role Matrix model-fetch states (per row)
loading · ready (role-aware recommended + others) · empty (manual entry) · error (+ retry) ·
manual/free-text committed.

### Acceptance criteria (V3)
- [ ] Clean profile lands in Setup Wizard, not Library.
- [ ] Setup completes only after ≥1 provider validates AND the five roles are confirmed.
- [ ] Dismissing setup shows the persistent “Connect your storyteller” banner (both CTAs + reason).
- [ ] New Story / open-to-play route the unconfigured user to setup with context; Import & browse stay.
- [ ] Provider order is the canonical 11 in Setup Wizard, Settings, and every Role Matrix picker.
- [ ] Electron Hub & NanoGPT have full cards (desc, abbr, placeholder, signup/help, all states).
- [ ] Custom endpoint has two separate fields (Base URL + masked key), URL validation, Validate
      connection, discovery, and manual fallback.
- [ ] Role Matrix Provider and Model are separate; models fetch live per provider (loading/empty/error).
- [ ] Two registers, tokens, and Ruling animation unchanged from V2.
