...
## v6 components (see 03-COMPONENT-STATES for full specs)
Forge **progress stepper** (discrete stages + active-call spinner + substep/attempt; no %) · **macro
compatibility warning** · **mechanic-source badge** (CARD/PERSONA/BLUEPRINT/CUE/GENERATED) · **XP
progress meter + rank-up event** (replaces success pips) · **entity-resolution notice** ·
**regeneration impact dialog** · **classifier-skipped notice** · **primary-provider badge +
Make-primary**. Attributes render 1–10 in the SYSTEM register with a locked-0 state. All reuse the
existing tokens and the STORY/SYSTEM register split unchanged.