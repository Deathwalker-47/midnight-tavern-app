...
Files/links: Play.dc.html, Library.dc.html, Wizard.dc.html, Overview.dc.html, Characters.dc.html, StorySettings.dc.html, CardCreator.dc.html (persona editor · simple), Personas.dc.html, Lorebook.dc.html, Settings.dc.html, RoleMatrix.dc.html, StoryBlueprint.dc.html, CharacterDossier.dc.html, DesignSystem.dc.html
...
## V3 · Providers & onboarding
Canonical provider order (everywhere): OpenRouter, Electron Hub, NanoGPT, OpenAI, Anthropic, Google, Mistral, DeepSeek, xAI, Groq, Custom endpoint. OpenRouter recommended. Onboarding = SetupWizard.dc.html (connect + confirm 5 roles, completion-gated); story creation = New Story Builder in Library (premise+persona / StoryBlueprint). Unconfigured Library shows persistent "Connect your storyteller" banner; model-dependent actions route to setup, local actions stay. Custom endpoint = two fields (Base URL + masked key) + validate + discovery + manual fallback. RoleMatrix = separate Provider/Model, live per-provider model fetch (loading/ready/empty/error).