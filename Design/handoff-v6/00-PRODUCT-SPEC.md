...
## v5 architecture - mode-aware (supersedes any universal-pipeline statement above)

**There is no universal five-role pipeline.** Roles activate **by stat system**:

- **No Stats story** -> per-turn path is *player message -> Narrator -> reply*. Classifier, Analyzer,
  Summarizer, Story AI are **dormant** (see the role-activation contract in `00-PRODUCT-DECISIONS`).
  Lorebook retrieval, persona injection, transcript assembly, safe formatting, and local navigation
  still work - none needs another model role.
- **Full Stats story** -> the full deterministic pipeline runs (Classifier -> gate -> resolver/ledger
  -> Narrator -> Analyzer; Summarizer at thresholds; Story AI only at forge/upgrade). Mechanical
  outcomes are deterministic; **models never write hard state**, including attributes.

Any earlier sentence implying every story calls Classifier/Analyzer/Summarizer/Story AI, that a
ruling mounts mid-stream after prose begins, that attributes are pending, or that a per-story
Narrator override exists, is **void in v5**. Stat systems are **No Stats** and **Full Stats** only.

---

## v6 additions (mode-aware, supersede v5 attribute statements)
Attributes are **1–10** (`scoreToModifier = score − 5`); mastery is **XP-based** (thresholds
100/250/625 ×2.5, engine-awarded); the rulebook may be **destructively regenerated** through a
warning-gated version boundary (immutable in ordinary play otherwise); a persisted **primary
provider** supplies UX defaults without overriding per-role bindings; narrated **scene entities**
become targetable via a local registry; a sealed **universal-action** foundation underlies every
Full Stats story; **SillyTavern macros** expand at assembly with an explicit support table. Two modes
(No Stats/Full Stats), No Stats = Narrator-only, one global Narrator, engine-only hard-state writes,
and no sixth model role all stand from v5.