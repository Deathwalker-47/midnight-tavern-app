# Midnight Tavern — Developer Handoff · V2
...
> **⭐ V2 — read `00-WHATS-NEW-V2.md` first.** It's the diff from V1: two new screens
> (Character Dossier, Role Matrix), a real interactive model/sampler configurator, global
> lorebooks + attach, the full Story Blueprint editor, persona attach, and Play message
> swipe/rewind. Everything else is unchanged from V1.
...
├── 00-WHATS-NEW-V2.md        ← ⭐ the V1→V2 diff — start here if you know V1
...
├── 02-STATES-AND-FLOWS.md    ← per-screen state matrix + flagship flows step-by-step
├── 03-IMPLEMENTATION-NOTES.md← how to read the prototype files + build guidance
├── screens/                  ← the 15 interactive prototype screens (open in a browser)
│   ├── Index.dc.html         ← screen directory / entry point
│   ├── Play.dc.html          ← THE core screen — turn engine, rulings, drawer, swipe/rewind
│   ├── Library.dc.html       ← story shelf + premise → forging; persona picker; import
│   ├── Wizard.dc.html        ← 3-step first-run setup (embeds RoleMatrix at step 3)
│   ├── Overview.dc.html      ← chapter timeline + arc document
│   ├── Characters.dc.html    ← living-card roster → "Open full profile"
│   ├── CharacterDossier.dc.html ← ★NEW deep read-only character profile (both registers)
│   ├── StorySettings.dc.html ← frozen rulebook + lorebook attach panel + persona row
│   ├── CardCreator.dc.html   ← persona editor (SIMPLE: name, role, description, voice)
│   ├── StoryBlueprint.dc.html ← ★NEW full story-card editor (SillyTavern-parity fields)
│   ├── Personas.dc.html      ← persona gallery
│   ├── Lorebook.dc.html      ← ★global lorebook library → entry editor
│   ├── Settings.dc.html      ← providers/keys, RoleMatrix, license
│   ├── RoleMatrix.dc.html    ← ★NEW model-role matrix + sampler panel (embedded component)
...
1. **See it move first.** Open `screens/Index.dc.html` in a modern browser (Chrome/Safari/Firefox).
   ...
   - In **Play**, type any message and press Enter to watch a full turn: prose streams, a
     **Ruling** artifact animates in (die settles → math → total counts up → verdict stamps),
     ...
     (denied → NPC roll → combat exchange → nat-20 crit with mastery advance). Hover the latest
     narrator message to see the **swipe** counter and **⋯** menu.
2. **Know V1 already? Read `00-WHATS-NEW-V2.md`** — it's the complete diff.
3. **Read `00-PRODUCT-SPEC.md`** for what each screen is and does.
4. **Read `01-DESIGN-SYSTEM.md`** before writing any UI — it defines the two-register
   system that the whole look depends on.
5. **Read `02-STATES-AND-FLOWS.md`** to implement the same state coverage and the flagship flows.
6. **Cross-check against `reference/`** — the original plans define the data model, the
